...
class MCState:
    ...
    def successors(self) -> List[MCState]:
        sucs: List[MCState] = []
        if self.boat:  # boat on west bank => from west to east
            if self.wm > 1:
                # 2m
        ...
        else:  # boat on east bank => from east to west
            if self.em > 1:
                #  2m
                sucs.append(MCState(self.wm + 2, self.wc, not self.boat))
            if self.em > 0:
...
def display_solution(path: List[MCState]):
    """
    记录最终状态的同时，迭代遍历解题步骤中的所有状态。它会查看最终状态与当前正在迭代状态之间的差异，
    ...
    for current_state in path[1:]:
        if current_state.boat:
            # 如果当前boat为true，表示当前在west，那么上一个状态必然是在east。
            # old — current = moved
