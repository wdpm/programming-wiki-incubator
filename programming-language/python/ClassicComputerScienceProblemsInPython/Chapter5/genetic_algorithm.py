...
class GeneticAlgorithm(Generic[C]):
    ...
    def __init__(self,
        ...
        # crossover 被选中繁殖的双亲生育出带有它们的混合基因的后代的概率，若无混合基因的后代，则后代只是其双亲的副本
        self._crossover_chance: float = crossover_chance
        self._selection_type: GeneticAlgorithm.SelectionType = selection_type
        # GeneticAlgorithm类需要操纵Chromosome的子类，因此，_fitness_key将因子类的不同而不同。
        # 为了能访问它，我们用type()来引用当前正待求适应度的Chromosome的某个子类
    ...
    def _reproduce_and_replace(self) -> None:
        ...
        # 因为new_population根据上面的计算，一定是偶数，因此如果出现 len(new_population) > len(self._population)
        # ，那么self._population之前是奇数，为了保证新一代和旧一代的数量一致，需要new_population -1
